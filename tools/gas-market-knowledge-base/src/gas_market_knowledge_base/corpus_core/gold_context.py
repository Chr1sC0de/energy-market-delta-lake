"""Shared gold Markdown rendering and citation validation."""
...
type ResolveDisplayPath = Callable[[str], Path]
...
def _from_display_path(path_text: str) -> Path:
...
def validate_gold_context(
    *,
    gold_dir: Path,
    index_path: Path,
    display_path: DisplayPath = _as_posix,
    resolve_display_path: ResolveDisplayPath = _from_display_path,
) -> GoldContextValidationResult:
    """Validate gold Market context pages against the silver chunk index."""
    page_paths = _gold_page_paths(gold_dir)
    if not page_paths:
    ...
    glossary_index_path = gold_dir / _GLOSSARY_INDEX_PATH
    ...
    for page_path in page_paths:
        ...
        errors.extend(
            _frontmatter_errors(
                ...
                resolve_display_path=resolve_display_path,
            )
        )
        errors.extend(
...
def _frontmatter_errors(
    ...
    *,
    ...
    if generated_path is not None and generated_path != display_path(page_path):
        errors.append(
    ...
    for chunk_id in source_chunk_ids:
        ...
        elif not resolve_display_path(chunk_path).exists():
            errors.append(
                f"{page_path} chunk {chunk_id!r} target missing: "
                f"{resolve_display_path(chunk_path)}"
            )
        ...
        elif not resolve_display_path(source_document_path).exists():
            errors.append(
                f"{page_path} chunk {chunk_id!r} source document target missing: "
                f"{resolve_display_path(source_document_path)}"
            )
...
def _source_citation_target_errors(
    ...
    *,
    ...
    if chunk_path is not None:
        expected_chunk_path = resolve_display_path(chunk_path).resolve()
    ...
    if source_document_path is None:
        ...
        expected_source_document_path = resolve_display_path(source_document_path)
