"""Shared silver Docling Hybrid chunks and retrieval index generation."""
    ...
    for row_number, row in enumerate(index_rows, 1):
        ...
        if chunk_id is not None:
            if chunk_id in chunk_ids:
                errors.append(
                    f"{index_path}:{row_number} duplicate chunk_id "
                    f"{chunk_id!r}; "
                    f"first seen at {chunk_ids[chunk_id]}"
                )
            else:
                row_path = resolve_display_path(_optional_text(row, "path") or "")
        ...
        if path_text is None:
            continue
        chunk_path = resolve_display_path(path_text)
...
def _chunk_frontmatter(
    ...
    *,
    ...
    source_document_path = display_path(entry.output_path)
    ...
    return {
        ...
        "chunk_id": chunk_id,
        "chunk_ordinal": chunk_ordinal,
        "generated_path": display_path(chunk_path),
        "path": display_path(chunk_path),
        "document_identity": entry.document_identity,
        "content_sha256": entry.content_sha256,
...
def _chunk_frontmatter_errors(
    ...
    *,
    ...
    if source_doc is None:
        ...
        source_doc_path = resolve_display_path(source_doc)
...
def _chunk_markdown_path(
    ...
    *,
    ...
    try:
        identity_parts = document_identity_parts(document_identity)
    except ValueError as e:
        raise SilverChunkInputError(str(e)) from e
