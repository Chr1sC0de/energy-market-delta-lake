...
path. **Gitflow delivery** is the default: Ralph integrates validated issue work
to `dev`, leaves the issue open with `agent-integrated`, and later **Promotion**
merges reviewed `dev` work to `main` while closing verified issues. **Trunk
delivery** remains available for small low-risk docs, tests, tooling, or script
changes that can integrate directly to `main` and close immediately.
