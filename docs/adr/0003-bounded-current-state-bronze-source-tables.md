...
Normal ingestion archives processed landing files when the helper reports a
table write, or when it reports a zero-row batch that leaves current state
unchanged. If the helper returns without writing a table for non-empty processed
rows, processed files remain in landing and the bronze materialization emits a
non-blocking WARN asset check. Zero-byte landing objects are deleted after the
write helper returns normally. Missing and unsupported selected keys remain in
landing and are reported by the same skipped-key check.
...
The archive bucket contains source files whose source-table bronze run either
reached an actual table write or processed a zero-row batch that required no
table change. Operators should inspect the skipped-key WARN asset check when
