...
# -----------------------------
#
# This script provides secure access to the jump server in the AWS environment.
# It retrieves the necessary SSH key pair from AWS SSM Parameter Store and
# establishes an SSH connection to the jump server.
#
# Usage:
#   DEVELOPMENT_ENVIRONMENT=dev NAME_PREFIX=energy-market ./ssh-jump-server.sh
#
# Parameters:
...
KEY_PAIR_ID_SSM_PATH="/$GROUP/dagster/jump-server/key-pair-id"
JUMP_SERVER_INSTANCE_ID_SSM_PATH="/$GROUP/dagster/jump-server/instance-id"
...
#     ╭────────────────────────────────────────────────────────────────────────────────────────╮
#     │                 get the jump server id and get the instance public dns                 │
#     ╰────────────────────────────────────────────────────────────────────────────────────────╯

JUMP_SERVER_INSTANCE_ID=$(aws ssm get-parameter --name "$JUMP_SERVER_INSTANCE_ID_SSM_PATH" | rg '"Value": "(.+)",' -r '$1' | tr -d '[:blank:]')

JUMP_SERVER_DNS_NAME=$(
...
echo "INFO: JUMP_SERVER_DNS_NAME=$JUMP_SERVER_DNS_NAME"
...
#     ╭────────────────────────────────────────────────────────────────────────────────────────╮
#     │                              now ssh into the jump server                              │
#     ╰────────────────────────────────────────────────────────────────────────────────────────╯

ssh -i ./.scratch/key-pair.pem "ec2-user@$JUMP_SERVER_DNS_NAME"
