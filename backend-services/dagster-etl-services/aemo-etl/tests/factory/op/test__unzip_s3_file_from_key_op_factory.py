...
def upload_files_to_s3(create_buckets: None, s3: S3Client) -> None:
...
def validate_unzipped_files(s3: S3Client, unzipped_files: dict[str, str]) -> None:
    for dict_ in unzipped_files:
        bucket = dict_["Bucket"]  # ty:ignore[invalid-argument-type]
        key = dict_["Key"]  # ty:ignore[invalid-argument-type]
...
class Test__unzip_s3_file_from_key_op_factory:
    ...
    @mark.parametrize("zip_file", zip_files)
    def test__different_bucket(self, s3: S3Client, zip_file: Path) -> None:
        unzipped_files = unzip_s3_file_from_key_op_factory(
            s3_source_bucket=LANDING_BUCKET,
    ...
    @mark.parametrize("zip_file", zip_files)
    def test__different_prefix(self, s3: S3Client, zip_file: Path) -> None:
        unzipped_files = unzip_s3_file_from_key_op_factory(
            s3_source_bucket=LANDING_BUCKET,
            s3_target_bucket=LANDING_BUCKET,
    ...
    @mark.parametrize("zip_file", zip_files)
    def test__different_prefix_and_bucket(self, s3: S3Client, zip_file: Path) -> None:
        unzipped_files = unzip_s3_file_from_key_op_factory(
            s3_source_bucket=LANDING_BUCKET,
            s3_target_bucket=BRONZE_BUCKET,
            s3_target_prefix="mockdatat/prefix_2",
