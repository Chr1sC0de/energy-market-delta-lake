...
def sensor_aemo_vicgas_jobs(context: SensorEvaluationContext, s3: S3Resource):
    ...
    for configuration in configurations:
        ...
        compact_and_vacuum_records = context.instance.get_run_records(
            RunsFilter(
                job_name=compact_and_vacuum_job_name,
                statuses=[
                    DagsterRunStatus.QUEUED,
                    DagsterRunStatus.NOT_STARTED,
                    DagsterRunStatus.STARTING,
        ...
        if (
            len(core_job_run_records) == 0
            and len(compact_and_vacuum_records) == 0
            and not has_job_failed(context.instance, asset_job_name)
        ):
