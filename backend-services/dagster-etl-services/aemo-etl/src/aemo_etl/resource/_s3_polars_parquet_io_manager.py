...
class S3PolarsParquetIOManager(IOManager):
    ...
    is set in the metadata. The s3_polars_parquet_io_manager_options should consist of
    an PolarsLazyFrameScanParquetParamSpec and PolarsLazyFrameSinkParquetParamSpec
    object e.g.
    ...
    def handle_output(self, context: OutputContext, obj: LazyFrame) -> None:
        ...
        assert definition_metadata is not None, (
            "writing to s3 requires both sink_parquet_options and scan_parquet_options"
            " to be set in asset metadata"
        )
        assert "s3_polars_parquet_io_manager_options" in definition_metadata, (
            "s3_polars_parquet_io_manager_options must be set in asset metadata"
        )

        assert (
            "sink_parquet_options in "
            "{definition_metadata['s3_polars_parquet_io_manager_options']}"
        )

        sink_parquet_options = definition_metadata[
            "s3_polars_parquet_io_manager_options"
        ].get("sink_parquet_options")

        assert isinstance(sink_parquet_options, PolarsLazyFrameSinkParquetParamSpec), (
            "'sink_parquet_options' must be of type "
            "'PolarsLazyFrameSinkParquetParamSpec'"
        )
        ...
        markdown_preview = (
            cast(DataFrame, obj.head().collect()).to_pandas().to_markdown()
        )
        ...
        output_metadata["dagster/row_count"] = MetadataValue.int(
            cast(DataFrame, obj.select(len()).collect()).item()
        )
    ...
    def load_input(self, context: InputContext) -> LazyFrame:
        ...
        assert (
            "scan_parquet_options in "
            "{definition_metadata['s3_polars_parquet_io_manager_options']}"
        )

        scan_parquet_options = definition_metadata[
            "s3_polars_parquet_io_manager_options"
        ].get("scan_parquet_options")

        assert isinstance(scan_parquet_options, PolarsLazyFrameScanParquetParamSpec), (
            "'scan_parquet_options' must be of type "
            "'PolarsLazyFrameScanParquetParamSpec'"
        )
