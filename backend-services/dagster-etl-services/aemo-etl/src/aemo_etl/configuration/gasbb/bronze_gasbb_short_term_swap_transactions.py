"""bronze_gasbb_short_term_swap_transactions - Bronze GASBB report configuration."""
...
def CONFIG() -> ReportConfig:
    ...
    return gasbb_config_factory(
        table_name="bronze_gasbb_short_term_swap_transactions",
        s3_file_glob="gasbbshorttermswaptransactions*",
        primary_keys=[
            "PeriodID",
            "State",
            "TransactionType",
        ...
        report_purpose="""
            These reports display short term gas transactions for each state/territory,
            excluding those transactions that are concluded through an AEMO operated
            exchange. Reports for VIC and QLD are updated weekly while others are
            updated monthly.
        """,
    )
