...
def CONFIG() -> ReportConfig:
    ...
    return gasbb_config_factory(
        ...
        schema_descriptions={
            ...
            """,
            "TransferIn": """
                Usage type. Only applicable to BB pipelines. Three decimal places is not
                shown if the value has trailing zeros after the decimal place.
            """,
            "TransferOut": """
                Usage type. Only applicable to BB pipelines. Three decimal places is not
                shown if the value has trailing zeros after the decimal place.
            """,
            "HeldinStorage": """
                Three decimal places is not shown if the value has trailing zeros after
                the decimal place.
            """,
            "FacilityId": "A unique AEMO defined Facility identifier.",
            "FacilityType": """
                The type of facility (e.g., BBGPG, COMPRESSOR, PIPE, PROD, STOR,
                LNGEXPORT, LNGIMPORT, BBLARGE).
            """,
            "CushionGasStorage": """
                The quantity of gas that must be retained in the Storage or LNG Import
                facility in order to maintain the required pressure and deliverability
                rates.
            """,
            "LastUpdated": """
        ...
        report_purpose="""
            The report shows Daily Production, Flow and Storage data aggregated by
            Facility Id for an outlook period.

            This report is updated daily and shows historic records back to Sep 2018.
            There is also a GASBB_ACTUAL_FLOW_STORAGE_LAST_31 variant that is updated
            within 30 minutes of receiving new data and shows records from the last 31
            days.

            The report can be filtered by: - State - Facility Type - Facilities
        """,
    )
