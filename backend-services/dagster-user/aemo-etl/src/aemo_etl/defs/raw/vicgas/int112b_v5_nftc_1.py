...
defs = df_from_s3_keys_definitions_factory(
    domain="vicgas",
    name_suffix="int112b_v5_nftc_1",
    glob_pattern="int112b_v5_nftc_1*",
    schema={
        "nftc_name": String,
    ...
    description=dedent("""
        ...
        given day in the reporting window. A report is produced each time an operational
        schedule (OS) is approved by AEMO. Therefore, it is expected that each day
        there will be at least 9 of these reports issued: - 5 being for the standard
        current gas day schedules - 3 being for the standard 1 ahead schedules - 1
        being for the standard 2 days ahead schedule. Each report contains details of
        the NFTCs that have applied and will apply to schedules run: - on the previous
        gas day - for the current gas day - for the next 2 gas days. Each NFTC has a
        unique identifier and applies to a single group of MIRNs (an injection MIRN and
        a withdrawal MIRN). Each row in the report contains details of one NFTC for one
        hour of the gas day, with hourly intervals commencing from the start of the gas
        day. This report will contain 24 rows for each NFTC for each gas day reported.
    """).strip("\n"),
