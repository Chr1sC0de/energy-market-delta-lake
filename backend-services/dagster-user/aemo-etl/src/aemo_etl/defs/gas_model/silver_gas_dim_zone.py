...
SOURCE_TABLES = [
    ...
    "bronze.gbb.bronze_gasbb_linepack_zones",
    "bronze.vicgas.bronze_int188_v4_ctm_to_hv_zone_mapping_1",
    "bronze.vicgas.bronze_int284_v4_tuos_zone_postcode_map_1",
    "bronze.vicgas.bronze_int259_v4_pipe_segment_1",
]
GBB_DEMAND_ZONE_MAPPING_KEY = AssetKey(
    ["bronze", "gbb", "bronze_gasbb_demand_zones_and_pipeline_connectionpoint_mapping"]
)
GBB_LINEPACK_ZONES_KEY = AssetKey(["bronze", "gbb", "bronze_gasbb_linepack_zones"])
...
def _gbb_linepack_zones(df: LazyFrame) -> LazyFrame:
    return df.select(
        source_system=pl.lit("GBB"),
        source_tables=pl.lit(["bronze.gbb.bronze_gasbb_linepack_zones"]).cast(
            pl.List(pl.String)
        ),
...
def _vicgas_linepack_zones(df: LazyFrame) -> LazyFrame:
    return df.select(
        source_system=pl.lit("VICGAS"),
        source_tables=pl.lit(["bronze.vicgas.bronze_int259_v4_pipe_segment_1"]).cast(
            pl.List(pl.String)
        ),
        zone_type=pl.lit("linepack_zone"),
