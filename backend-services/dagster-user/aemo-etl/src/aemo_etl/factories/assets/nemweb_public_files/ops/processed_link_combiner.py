...
from aemo_etl.factories.assets.nemweb_public_files.models import (
    ProcessedLink,
)
...
def build_process_link_combiner_op(
    ...
    def _op(
        ...
        df = processed_link_combiner.combine(
            context, processed_links, schema, surrogate_key_sources
        )
        return df
...
class S3ProcessedLinkCombiner(ProcessedLinkedCombiner):
    def combine(
        ...
        for results in processed_links:
            if results is not None:
                filtered_links.extend(results)
                ...
                dict(
                    {
                        ...
                        "target_s3_href": [
                            link.target_s3_href for link in filtered_links
                        ],
                        "target_s3_bucket": [
                        ...
                        "target_s3_name": [
                            link.target_s3_name for link in filtered_links
                        ],
                        "target_ingested_datetime": [
                            link.target_ingested_datetime for link in filtered_links
                        ],
                        # make a dummy surrogate key to be processed post op run
                        "surrogate_key": [""] * len(filtered_links),
                    }
                ),
                schema=schema,
            )
