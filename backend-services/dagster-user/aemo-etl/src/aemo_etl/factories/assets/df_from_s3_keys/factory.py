...
from polars import LazyFrame, concat, lit
...
def df_from_s3_keys_asset_factory(
    ...
    **asset_kwargs: Unpack[AssetDefinitonParamSpec],
    ...
    if "dagster/column_schema" not in asset_kwargs:
        asset_kwargs["metadata"]["dagster/column_schema"] = get_metadata_schema(
            schema, schema_descriptions
        )

    asset_kwargs.setdefault("kinds", {"table", "deltalake"})

    asset_kwargs["metadata"]["surrogate_key_sources"] = surrogate_key_sources
