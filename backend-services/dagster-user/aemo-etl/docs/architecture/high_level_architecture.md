...
STTM discovery is intentionally root-only for public CSV reports under
`REPORTS/CURRENT/STTM`. It excludes `CURRENTDAY.*`, `DAYNN.ZIP`, subfolders
...
The source-table modules under `src/aemo_etl/defs/raw/gbb`,
...
with `bronze`. The current-state decision applies to the GBB, STTM, and VICGAS
source-table assets generated by `df_from_s3_keys`; it does not apply to
`bronze_nemweb_public_files_*` discovery/listing assets, which record discovered
file metadata, and it does not apply to `unzipper_*` assets, which expand zip
payloads and archive successful zip inputs.

The STTM source-table bronze coverage spans `INT651` through `INT669`, including
`bronze/sttm/bronze_int651_v1_ex_ante_market_price_rpt_1` through
`bronze/sttm/bronze_int669_v1_settlement_version_rpt_1`. The compact manifest
declares every STTM report column as `String` and uses the normal ingestion
metadata, `surrogate_key`, `source_file`, and `source_content_hash` conventions.

Source-table bronze semantics:

- bronze stores bounded current state for source-table assets, not
  append-history rows
- `surrogate_key` is generated from the source table's configured key columns
- `source_content_hash` is a SHA-256 hash over declared source columns,
  excluding ingestion metadata, `surrogate_key`, `source_file`, and
  `source_content_hash`
- each micro-batch keeps the maximum `source_file` row per `surrogate_key`
- Delta merge uses `source.surrogate_key = target.surrogate_key`
- matched rows update only when `source_content_hash` changes or the target hash
  is missing
...
- `vicgas_event_driven_assets_sensor`, `gbb_event_driven_assets_sensor`, and
  ...
  - suppress repeated launches after a failed job at the same job tags, but allow a retry when retry-relevant tags such as ECS CPU or memory change
