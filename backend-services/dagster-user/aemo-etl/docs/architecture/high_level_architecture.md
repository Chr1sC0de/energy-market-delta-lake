...
The STTM source-table bronze coverage spans `INT651` through `INT659`, including
`bronze/sttm/bronze_int651_v1_ex_ante_market_price_rpt_1` through
`bronze/sttm/bronze_int659_v1_bid_offer_rpt_1`. The compact manifest declares
every STTM report column as `String` and uses the normal ingestion metadata,
`surrogate_key`, `source_file`, and `source_content_hash` conventions.
